source(here::here(".//stochasticModelsSimplifiedExtensionSGEscape//InfDissTransModelFuncs.R"))
library(parallel)
  ...
  simEachConc <- tryCatch ({ 
    simEachConc <-    lapply(startingVirus,function(conc){
      repSim <- lapply(1:100,function(y){ 
        ...
        if(as.character(x[9])==T){
          sc <-  as.numeric(x[8])/as.numeric(x[7])
          sh <- as.numeric(x[7])^2/ as.numeric(x[8])
          rate <- 1/sc 
          es <- rgamma(1,shape=sh,rate=rate)
        }else{
          es <- as.numeric(x[7]) 
        }
